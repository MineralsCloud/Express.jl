    ...
    while true
        ...
        if !isdone
            ...
            if hasfield(T, field)
                ...
                nml = set(nml, PropertyLens{field}(), parse(fieldtype(T, field), readline(terminal)))
                continue
            end
...
function namelist_helper(terminal::TTYTerminal, ::Type{T}) where {T<:PWscf.SystemNamelist}
    ...
    celldm = map(x->parse(Float64, x), split(strip(readline(terminal), ['[', ']', '\n']), ",", keepempty = false))
    ...
    print(terminal, "Please input the kinetic energy cutoff (Ry) for wavefunctions `ecutwfc`: ")
    ecutwfc = parse(Float64, readline(terminal))
    print(terminal, "Please input the Kinetic energy cutoff (Ry) for charge density `ecutrho`: ")
