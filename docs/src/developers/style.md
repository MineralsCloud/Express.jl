...
!!! info
    The style guide is always a work in progress, and not all Express code
    follows the rules. When modifying Express, please fix the style violations
...
`Express` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/Express.jl/blob/master/.JuliaFormatter.toml).

To format your code, `cd` to the `Express` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `Express` have
